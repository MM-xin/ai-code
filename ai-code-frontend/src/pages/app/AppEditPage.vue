    ...
    <div class="edit-container">
      <a-card title="基本信息" :loading="loading">
        <a-form :model="formData" :rules="rules" layout="vertical" @finish="handleSubmit" ref="formRef">
          ...
          <a-form-item v-if="formData.deployKey" label="部署密钥">
...
const formData = reactive({ appName: '', cover: '', priority: 0, initPrompt: '', codeGenType: '', deployKey: '' })
...
const fetchAppInfo = async () => {
  ...
  } catch (e) { message.error('获取应用信息失败'); router.push('/') } finally { loading.value = false }
}
...
const handleSubmit = async () => {
  ...
  try {
    let res
    if (isAdmin.value) {
      res = await updateAppByAdmin({ id: appInfo.value.id, appName: formData.appName, cover: formData.cover, priority: formData.priority })
    } else {
  ...
  } catch (e) { message.error('修改失败') } finally { submitting.value = false }
}
...
const openPreview = () => {
  if (appInfo.value?.codeGenType && appInfo.value?.id) {
    window.open(getStaticPreviewUrl(appInfo.value.codeGenType, String(appInfo.value.id)), '_blank')
  }
}
