  ...
  <div id="appManagePage">
    ...
    <a-table :columns="columns" :data-source="data" :pagination="pagination" @change="doTableChange" :scroll="{ x: 1200 }">
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.dataIndex === 'codeGenType'">{{ formatCodeGenType(record.codeGenType) }}</template>
...
import { listAppVoByPageByAdmin, deleteAppByAdmin, updateAppByAdmin } from '@/api/app'
...
const editApp = (app: API.AppVO) => { router.push(`/app/edit/${app.id}`) }
